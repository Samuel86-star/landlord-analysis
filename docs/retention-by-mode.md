...
-- 完整设计见 dws/dws_ddz_daily_play_by_mode.md
...
-- 4. 同玩法留存 flag（基于 dws_ddz_daily_play_by_mode，只看注册日之后）
...
day_flags_mode AS (
    ...
    FROM new_user_reg r
    LEFT JOIN tcy_temp.dws_ddz_daily_play_by_mode a ON r.uid = a.uid AND a.dt > r.reg_date
...
day_flags_global AS (
    ...
    FROM new_user_reg r
    LEFT JOIN tcy_temp.dws_ddz_daily_play a ON r.uid = a.uid AND a.dt > r.reg_date
    GROUP BY r.uid
)
-- 5. 聚合最终分玩法宽表
SELECT
...
> **创建日期**：2026-03-25
> **更新说明**：
> - v2.0：重构 DWS 层架构（对齐主文档 v3.0）；修正字段名（`room_base`/`diff_money` 等）；倍数字段改为直接读列；新增 `device_type` 维度；`dws_ddz_daily_play_by_mode` 时间上限延至 20260508；同玩法留存新增 Day30 指标；修正 `day_flags_mode` 添加 `a.dt > r.reg_date` 限制
