...
> ⚠️ 实际存储带玩法编号前缀（2026-09-17 实测 game_id=53 全量），**等值匹配须带前缀**（`game_rule = 'A:经典'`，裸 `'经典'` 会查空）；模糊匹配用 `LIKE '%经典'`。
...
> **初始化数据**：**已灌数据**。2026-09-07 补充经典新手场 1404（移动端和小游戏开放，与仅 PC 开放的 420 对应）；`game_rule` 取值 经典 / 不洗牌 / 癞子 / 积分 / 好友房；`room_level` 取值 练习房 / 新手房 / 初级房 / 中级房 / 高级房 / 大师房 / 宗师房（各玩法 ladder 独立按 A-G 编号，语义看中文名后缀）。
