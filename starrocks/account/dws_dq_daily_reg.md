...
通过 LEFT JOIN `dws_dq_daily_login` 获取渠道号和平台分组，再 LEFT JOIN `dq_channel_category_map` 获取渠道分类标签：
...
1. `reg_date` 字段为 DATE 类型（格式：YYYY-MM-DD），`reg_datetime` 为 datetime 类型
2. 查询时间段时使用 `reg_date` 或 `reg_datetime` 字段，注意 `reg_date` 为 DATE 类型（如：`reg_date BETWEEN '2026-02-10' AND '2026-02-10'`）
