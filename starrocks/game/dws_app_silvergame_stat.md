...
| 类型 | DWS 层聚合表（每日增量） |
| 描述 | APP 端用户每日游戏统计表，仅汇总银子玩法的金流与参与度，不含倍数 / 炸弹等玩法体验指标。510K（多轮玩法）金流并入此表，玩法体验信号见 `dws_app_gamemode_stat` |
...
为了让金流归因独立于玩法变量，本表只承担**金流 + 参与度**这一维度，玩法体验维度交由 `dws_app_gamemode_stat`（uid × dt × play_mode）。

### 与 dws_app_gamemode_stat 的分工

| | dws_app_silvergame_stat（本表） | dws_app_gamemode_stat |
| ---- | ---- | ---- |
...
| 4 | 积分（PC 端） | 积分 | ❌ 走 gamemode_stat |
| 5 | 比赛（APP/小游戏端） | 积分 | ❌ 走 gamemode_stat |
| 6 | 好友房 | 积分 | ❌ 走 gamemode_stat |
...
> **设计取舍**：本表移除了 510K 专属字段（`total_settle_rounds` / `avg_settle_rounds` / `outcome_gdp` / `max_settle_round_single`），510K 体验信号统一由 `dws_app_gamemode_stat`（play_mode=7）承载，保持本表的纯金流边界。
...
1. **玩法过滤**：仅汇总银子玩法（`play_mode IN (1, 2, 3, 7)`），积分玩法（4 / 5 / 6）请走 `dws_app_gamemode_stat`
...
(510K，多轮，game_id=521)
                                   └─ 同时写入 dws_app_gamemode_stat（玩法体验, uid×dt×play_mode）
                                       play_mode = 1, 2, 3, 7
```
...
### 4. 与 gamemode_stat 联用：金流画像 + 玩法体验

```sql
-- game_stat 给整体金流画像，gamemode_stat 给玩法体验
SELECT
    g.play_mode,
    CASE
...
FROM tcy_temp.dws_app_silvergame_stat s
JOIN tcy_temp.dws_app_gamemode_stat g
...
> - 玩法体验维度走 `dws_app_gamemode_stat`，倍数分桶用 NTILE(4) 分位分桶
...
> **v4.2 变更**：与 gamemode_stat 联动调整，gamemode_stat 倍数分桶从硬编码阈值（≤6 / 6\~24 / >24）改为 NTILE(4) 分位分桶
> **v4.1 变更**：移除 crazyddz_\* 专属字段（`total_settle_rounds` / `avg_settle_rounds` / `outcome_gdp` / `max_settle_round_single`），510K 体验信号统一由 gamemode_stat（play_mode=7）承载，保持本表的纯金流边界
> **v4.0 变更**：新建本表，替代 `dws_ddz_app_game_stat`（旧表含 app_code 与硬编码倍数阈值），510K 金流并入

> **设计文档**：详见 [dws_app_silvergame_stat_design.md](dws_app_silvergame_stat_design.md)
