...
按天 `DELETE + INSERT`（幂等可重跑），脚本：[`batch_insert_allgame_stat.py`](../../ops/py/batch_insert_allgame_stat.py)
...
ddz_agg AS (
    SELECT
        ...
        COUNT(CASE WHEN g.bomb_bet <= 1 THEN 1 END) AS bomb_0_games,
        COUNT(CASE WHEN g.bomb_bet DIV 2 = 1 THEN 1 END) AS bomb_1_games,
        COUNT(CASE WHEN g.bomb_bet DIV 2 = 2 THEN 1 END) AS bomb_2_games,
        COUNT(CASE WHEN g.bomb_bet DIV 2 >= 3 THEN 1 END) AS bomb_3plus_games,
