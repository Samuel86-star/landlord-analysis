#!/usr/bin/env python3
"""Batch backfill tcy_temp.dws_app_scoregame_stat via DELETE + INSERT, day by day.

Source: dws_ddz_daily_game (play_mode IN 4,5,6 积分玩法)
Target: tcy_temp.dws_app_scoregame_stat (uid × dt 积分玩法参与度+胜负，无金流)
仅 APP 端真人。积分玩法仅来自 dws_ddz_daily_game，无 510K。

Usage:
    py -3 -u .\batch_insert_app_scoregame_stat.py --start 20260617 --end 20260617
    py -3 -u .\batch_insert_app_scoregame_stat.py --start 2026-03-01 --end 2026-06-10 --dry-run
"""
from backfill_runner import run_backfill

INSERT_TEMPLATE = """INSERT INTO tcy_temp.dws_app_scoregame_stat
WITH ranked AS (
    SELECT *,
...
DELETE_TEMPLATE = (
    "DELETE FROM tcy_temp.dws_app_scoregame_stat "
    "WHERE app_id = {app_id} AND dt = '{dt}'"
)

CHECK_TEMPLATE = (
    "SELECT COUNT(*) AS cnt FROM tcy_temp.dws_app_scoregame_stat "
    "WHERE app_id = {app_id} AND dt = '{dt}'"
)
...
if __name__ == "__main__":
    run_backfill(
        table_label="dws_app_scoregame_stat",
        delete_template=DELETE_TEMPLATE,
        insert_template=INSERT_TEMPLATE,
        check_template=CHECK_TEMPLATE,
        depends_on=("dws_ddz_daily_game",),
    )
